...
- 固定内置视图按「全部 → 默认收藏夹 → 稍后再看」排列，再显示自建收藏夹。**没有「未分类」视图**（2026-09-23 按用户要求移除，见[决策记录](../decisions/2026-09-23-移除未分类视图.md)）：列表侧栏与图标网格都不再提供入口。后端 `stats.unclassified` 与 `GET /items?unclassified=true` 保留为数据能力，只是界面上没有入口
