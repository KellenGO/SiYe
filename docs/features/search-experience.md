...
- **单独获取某一个平台**（搜索范围那栏每颗平台胶囊右边的 ⟳）：只跑这一颗药丸对应的平台，
  结果**并进**当前结果，其它平台不重搜。入口在 `SearchBar` 的 `onPlatformFetch`，
  实现在 `useSearchExperience` 的 `handleFetchPlatform`：`POST /api/search/jobs` 只带这一个平台，
  本轮属于换批会话时带上 `continue_from` → 后端把该平台并进会话（**新平台允许加入**，
  见下一条），完成后用 `mergeSinglePlatformAppend` 合并。
  和「单平台重试」（`PlatformStatus` 上的重试按钮）的区别是合并方式：
  重试是**整体替换**（失败平台本来就没结果），这里是**追加/更新**——
  该平台原有的内容不会因为补了一次就消失。
  **平台是否勾选都不影响**：取消勾选某平台后想补看它的结果，直接点这个按钮即可，不用整组重搜；
  也**不会自动把它勾回来**（搜索范围仍是用户自己说了算），**不写搜索历史**。
  只有本轮已有搜索时才出现（它需要一个关键词作为依据）。
  失败时提示说的是"「小红书」这次没有取到内容"，不会谎称"已自动取消勾选"（它本来就没勾）。
- **换批时可以新增平台**：`POST /api/search/jobs` 带 `continue_from` 时，如果请求的平台不在
  当前探索会话里，后端**把它并进会话**（它自己的进度从零开始），而不是报"换批不能新增平台"。
  这样"单独获取某个平台"补进来的平台成为会话一员，原会话继续有效——之后「换一批」照常可用。
  取舍见 `docs/decisions/2026-09-19-单独获取某个平台.md`。
