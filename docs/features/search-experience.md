...
- 平台勾选默认行为（2026-09-18 起）：**首次进入、没有存过偏好时，四个平台都不预选（空集）**，由用户亲手勾选——按钮是勾选框，不是"能不能用"的状态灯。一旦用户勾选过任一平台、偏好落到 localStorage，后续始终维持"至少一个平台"：UI 层允许手动取消到零，但提交搜索时统一提示"先勾选至少一个平台"，不再区分"首次"与"已勾过又取消"两套规则。注意界限：`readPlatformPref` 在**存储键不存在**、或存的是**显式空数组**时都返回空集（首屏会把"还没有偏好"当空集写回存储，读回来必须仍是零勾选——这一点 2026-09-23 修过，见 `docs/decisions/2026-09-23-平台偏好空数组语义.md`）；只有**非数组 / 全部是非平台值**这类损坏值才回退全选，避免把用户卡住。偏好一旦落地，reducer 的 `platform_pref_set` 传入空集会保留原偏好（不把已有偏好清空）。
