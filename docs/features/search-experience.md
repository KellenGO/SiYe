...
- **首页 / 结果页是两个视图，可以来回切（结果不删）**：结果页在**搜索框左侧**有一个
  **主题色圆角**的「← 返回首页」按钮（`.btn.primary.home-back`，在 `.search-zone` 里、
  `SearchBar` 之前），点了只切视图 —— **不清结果、不清任务、不动勾选**；
  带着结果回到首页时，搜索框下方会出现「查看上次结果（N 条）」回到结果页
  ...
  文案 `search.backToHome` / `search.viewLastResults`。
