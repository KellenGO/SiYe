...
- **单独获取某一个平台**：搜索范围那一栏，每颗平台胶囊右边有一个 ⟳ 按钮，
  点它只重跑这一个平台（用本轮的关键词），结果合并进当前结果、其它平台不动。
  入口在 `SearchBar` 的 `onPlatformFetch`，实际走的是既有的「单平台重试」链路
  （`useSearchExperience` 的 `handleRetry` → `POST /api/search/jobs` 只带一个平台 →
  `mergeSinglePlatformRetry` 整体替换该平台分组）。
  **平台是否勾选都不影响**：取消勾选某平台后想补看它的结果，直接点这个按钮即可，
  不用整组重搜；也**不会自动把它勾回来**（搜索范围仍是用户自己说了算）。
  只有当本轮已有搜索时才出现（它需要一个关键词作为依据）。
