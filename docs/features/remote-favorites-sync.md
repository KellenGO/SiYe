...
把各平台收藏归档到本机 SQLite。B站和知乎的已接入收藏夹接口不设产品侧总条数上限；小红书当前仅有单一「收藏笔记」入口，抖音尚无可验证账号身份。**同步所选平台 / 继续只新增**（读到本机已有的内容段就停）；高级操作中的**重置同步直接覆盖所选且成功完整读取的平台本机归档**。两者都不修改平台收藏。

## 代码入口

| 职责 | 位置 |
|---|---|
| 持久化与重置事务 | `api/services/remote_favorites_store.py` 的 `replace_platform_from` |
...
| 平台分页来源、增量停点、临时重置库 | `aggregate_search/favorites_sync.py` 的 `synchronize_favorites` 与各 `*FavoritesSource` |
...
- 面向普通用户的跨平台收藏说明直接说「只读取平台收藏，不会添加、删除或移动」，并提示可另存本地收藏；不以「镜像」解释功能。普通同步仍保留旧内容，重置同步按下面的独立规则处理。
...
- **普通同步未取到的旧内容保留**：平台没有一致性快照，"这次没出现"不等于用户取消了收藏。日常同步只把新出现的内容入库，不因本轮没看到而删除旧记录。
- 同一内容在多个远端收藏夹出现时保存多份归属，只归档一份内容。
- B站按真实 UID 隔离；历史上没带账号的同步结果留在 `default` 分组里，和普通收藏一样展示（页面不再有"账号未确认"这类需要用户理解的状态）。
- 普通同步下，旧 `default` 归档和已识别账号归档可能包含同一远端内容；两份原始记录都保留，但全量快照按 `(platform, content_id)` 合并后才交给页面。页面也会再做一次同身份防御性去重，避免旧响应或异常数据造成跨页签卡片残留。
- **一键同步 = 增量**（2026-09-22 调整，替换原"完整核对 + 缺失确认"）：逐收藏夹从第 1 页开始往后读，连续 **5 条**在该账号、该收藏夹的既有基线中出现，就停止当前收藏夹并继续下一个。连续计数可跨页，新增会清零；本轮刚保存的内容不能成为停点。
  ...
  - 代价是**只看头部**：感知不到旧内容的深处删除/移动。这属于预期（未取到的旧内容保留），需要覆盖本机记录时使用「重置同步」。
  - 增量默认启用（`incremental_verified = True`，用户明确要求"不用每次重拉全部"）。**真实账号的排序稳定性仍未被证实**（见「已知坑」），这是接受的取舍。
- **「重置同步」是独立入口**：`sync_mode="reset"` 先在临时库完成一个平台的全量读取，成功后用一笔本机事务替换该平台所有旧账号与旧内容；失败、未登录或取消则保留该平台原归档。其他平台与个人本地收藏不参与替换。旧 `full` 模式保留为内部兼容路径，不再由用户按钮触发。取舍见[重置同步决策](../decisions/2026-09-23-重置同步边界.md)。
- 收藏页保留主要同步按钮；「重置同步」在默认收起的「高级操作」里，按钮旁只留「将读取所选平台，并直接覆盖本地跨平台收藏」，确认前不发请求。详细边界留在帮助页、`docs/使用说明.md` 和网页指南。历史 `default` 账号若有无法区分平台的共享收藏夹归属，重置会拒绝提交而保留旧归档。
...
- 遇限流 / 验证码 / 登录失效就停止该平台并显示原因；普通同步保留已取内容，重置同步丢弃临时结果、保留原归档。**不自动重试、不降数量试探**。
...
- 同步期间可点击「取消同步」：停止尚未完成的平台；普通同步保留已取内容，重置同步保留未完成平台的原归档。已成功完成重置的平台不会撤销；取消请求失败时保留重试入口。
- 同步按钮同时显示加载圆圈与「正在同步 · 取消」，状态和取消操作始终可见；创建任务时显示「正在启动同步」，提交取消后显示「正在取消」。平台进度使用「同步中」。
- 平台状态对用户显示「本次同步」和「已保存」数量；它不是新增量。每个收藏夹仍持久记录安全诊断：模式、是否已有基线、请求页数、返回条数与末页/五条命中/取消/异常停止原因，不保存 Cookie、令牌或完整响应，这些实现信息不在日常界面展示。
...
- 平台没有快照或变更日志保证。扫描会校验响应结构、重复内容、收藏夹前后变化及首部变化，但无法证明扫描期间完全没有深处变化。中间短页以有效 `has_more` 继续读取。**增量只看头部**：旧内容的删除、移动不会被发现；重置同步以成功读到的列表覆盖本机记录。
...
浏览器侧：`scripts/remote_favorites_smoke.py`（打开页面只读本机缓存、平台页签切换、一次 100 条 + 显示更多、日常同步走 auto、重置确认后走 reset、高级区域文案与窄屏）。

`tests/test_bilibili_favorites_sync.py` 覆盖分页超过 100、恢复、分页漂移、账号隔离、逐收藏夹增量算法及 worker 协议。pytest 必须使用新的项目内 `--basetemp`。

前端运行 `npm run test:search` 与 `npm run build`，再运行 `scripts/remote_favorites_smoke.py`：临时数据库、隔离浏览器、拦截外部请求，验证平台页签、显示更多及 auto/reset 两条入口。`scripts/probe_bilibili_favorites.py` 是显式真实只读探测，只取最多两页，不写归档；不作为常规测试运行。
