...
- **归档列表的排序只由服务端决定**（2026-09-21）：`ORDER BY platform, id DESC` —— 按平台分组、组内最近入库在前，翻页时同一平台连续。归档页给 `ResultTabs` 传 `disableSort`（否则会被前端的「综合排序」重排）和 `hidePlatformTabs`（那排平台页签只过滤**当前页**，而分页是全库的，留着会让人误以为"翻页是按平台分的"）。要按平台看就用归档区的平台筛选，分页会跟着限定在该平台内。
