...
（读到本机已有的内容段就停），另有「重新同步全部」用来彻底对齐。其他平台仍每次最多 100 条。
...
- 面向普通用户的跨平台收藏说明直接说「只读取平台收藏，不会添加、删除或移动」，并提示可另存本地收藏；不以「镜像」解释功能。同步与保留行为未变。
...
- **一键同步 = 增量**（2026-09-22 调整，替换原"完整核对 + 缺失确认"）：逐收藏夹从第 1 页开始往后读，连续 **5 条**在该账号、该收藏夹的既有基线中出现，就停止当前收藏夹并继续下一个。连续计数可跨页，新增会清零；本轮刚保存的内容不能成为停点。
  ...
  - 代价是**只看头部**：感知不到旧内容的深处删除/移动。这属于预期（未取到的旧内容保留），需要彻底对齐时用「重新同步全部」。
  - 增量默认启用（`incremental_verified = True`，用户明确要求"不用每次重拉全部"）。**真实账号的排序稳定性仍未被证实**（见「已知坑」），这是接受的取舍。
- **「重新同步全部」是兜底入口**：`sync_mode="full"` 从头翻到尾，只校正本次已完整读到的收藏夹；完整目录没再列出的旧夹标为「平台暂未找到」，但仍保留其内容与只读入口。目录失败或局部夹失败绝不清空旧归属。
...
- 平台没有快照或变更日志保证。扫描会校验响应结构、重复内容、收藏夹前后变化及首部变化，但无法证明扫描期间完全没有深处变化。中间短页以有效 `has_more` 继续读取。**增量更是只看头部**：旧内容的删除、移动不会被发现（这是接受的代价，「重新同步全部」用来兜底）。
...
浏览器侧：`scripts/remote_favorites_smoke.py`（打开页面只读本机缓存、平台页签切换、一次 100 条 + 显示更多、一键同步走 auto、重新同步全部走 full、确认归档工作台已撤掉）。
