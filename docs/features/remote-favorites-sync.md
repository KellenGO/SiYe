...
- 收藏页保留主要同步按钮；「重置同步」在默认收起的「高级操作」里，确认前不发请求。旁边问号直达应用内帮助页的「高级功能说明」区块；同一说明也在 `docs/使用说明.md` 和网页指南。历史 `default` 账号若有无法区分平台的共享收藏夹归属，重置会拒绝提交而保留旧归档。
...
浏览器侧：`scripts/remote_favorites_smoke.py`（打开页面只读本机缓存、平台页签切换、一次 100 条 + 显示更多、日常同步走 auto、重置确认后走 reset、问号链接与窄屏）。
