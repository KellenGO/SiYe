...
- 审计：旧「重新同步全部」走 `full`，扫描中直接写入本机正式库，只校正已完整读取收藏夹的归属，旧跨平台内容仍保留；不能称为覆盖。现在保留旧 `full` 内部兼容路径，用户操作单独走 `reset`。
- 交付：日常「同步所选平台 / 继续」保持只新增；「重置同步」放进收藏页默认收起的高级操作，先确认再发请求。重置按平台先读到临时库，成功后单事务替换该平台本机跨平台归档；失败、未登录或取消保留原数据，未选平台与个人本地收藏不参与。无法安全区分平台的混合旧账号拒绝覆盖。问号直达应用内帮助的独立「高级功能说明」，两份使用说明、教程、功能 wiki、决策和更新日志同步口径。
- 验证：前端 `npm run test:search` 373/373、`npm run build` 通过，`webui/dist` 按本次 UI 源码重建；相关 pytest 164/164 通过（全新项目内 basetemp，含文档与 UI 契约），覆盖只新增、所选平台替换、未选/个人收藏保留、失败和取消保留、混合旧账号拒绝覆盖；`scripts/remote_favorites_smoke.py` 与 `scripts/getting_started_smoke.py` 在隔离浏览器通过，覆盖确认取消、问号直达、键盘与 390px 无横向溢出。`git diff --check` 通过。烟测仅用临时库和模拟接口。
- 未做：未跑全量后端 pytest，未用真实平台账号或真实收藏验证，也未重建 EXE。源码与本地前端构建一致；已打包 EXE 仍是旧版本。提交定位：`git log --all --grep=V1-RESET-SYNC-20260923`；已集成主工作区 `master`，本任务修改范围释放。
- 提交前主工作区另出现未由本任务产生的 `README.md`、`site/index.html`、图片和 `site/维护说明.md` 改动；已核对并排除在本任务暂存及提交外，归属仍待对应负责人核实，本记录不代其交接。

### PRODUCT-SHOTS-20260923 — 更新产品截图

- 负责人：Codex / 当前会话；主工作区 `MediaCrawler-main`，`master`，起始 HEAD `65899f1`。仅负责 README、推广落地页截图及截图维护说明，不涉及产品逻辑。
- 交付：用用户提供的实拍图替换首页、搜索、收藏列表、收藏图标、跨平台收藏、历史、账号与登录、外观设置共 8 张截图；只遮挡内容作者昵称，保留封面与人物画面。同步更新 `README.md`、`site/index.html`、`site/维护说明.md`，并重新生成落地页分享图 `site/og.png`。
- 验证：8 组 `site/` 与 `docs/images/` 截图均为 2550×1233 且文件一致；目视检查遮挡位置与落地页分享图；相关文档 `git diff --check` 通过。未运行测试（仅文档和图片资产变更）。
- 集成：截图文件、README 与落地页改动单独提交；本文件中的 RESET-SYNC 交接属于另一组现有改动，不纳入本任务提交。截图更新不改变源码、构建产物或运行版本。
