...
function sources(): DiagnosticSources {
  return {
    health: { state: "available", http_status: 200, data: { api_version: "0.2.2", browser_available: true, environment_status: "ok" } },
...
test("报告仅保留已知字段，剔除所有响应中的个人内容与任意错误原文", () => {
  ...
  input.health.data = { api_version: "0.2.2", browser_backend: `C:/Users/${secret}/chrome.exe`, secret };
  ...
  const text = buildSupportReport(input, "0.2.2", `Windows Edg/140.0 ${secret}`);
...
test("缺失、类型错误和非法数字显示未知，空任务不冒充成功", () => {
  ...
  const report = JSON.parse(buildSupportReport(input, "0.2.2", ""));
...
test("只请求四个本机只读端点，单项失败仍生成其余报告", async () => {
  ...
  const fetcher: typeof fetch = async (url, options) => {
    ...
    return Response.json(path.endsWith("current") ? null : { api_version: "0.2.2" });
  };
...
test("后端离线时，报告仍包含前端版本和浏览器类型", async () => {
  ...
  const report = JSON.parse(buildSupportReport(await collectSupportDiagnostics(fetcher), "0.2.2", "Windows Chrome/140.0"));
  assert.equal(report.ui_version, "0.2.2");
